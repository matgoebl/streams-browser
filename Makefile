...
install-dry:
	helm install --dry-run --debug $(HELM_OPTS) --namespace=$(NAMESPACE) $(NAME) ./streams-browser-helm

install: image basicauth_secret_update
	helm lint ./streams-browser-helm
	helm upgrade --install $(HELM_OPTS) --namespace=$(NAMESPACE) $(NAME) ./streams-browser-helm
